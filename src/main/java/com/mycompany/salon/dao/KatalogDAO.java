...
public class KatalogDAO {
    ...
    public boolean updateInfo(Katalog k) {
        String sql = "UPDATE katalog SET nama_layanan=?, harga=?, deskripsi=?, durasi_menit=? WHERE id=?";
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            pstmt.setString(3, k.getDeskripsi());
            pstmt.setInt(4, k.getDurasiMenit());
            pstmt.setInt(5, k.getId()); // ID di parameter terakhir
    ...
    public boolean updateRating(Katalog k) {
        String sql = "UPDATE katalog SET rating=?, jumlah_ulasan=? WHERE id=?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {

...
public class PelangganDAO extends PenggunaDAO {
    ...
    public boolean update(Pelanggan p) {
        String sql = "UPDATE users SET nama=?, email=?, telepon=? WHERE id=?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
    ...
    public Pelanggan findById(int id) {
        String sql = "SELECT * FROM users WHERE id = ?"; // Pastikan nama tabel benar
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, id);
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    p.setNama(rs.getString("nama"));
                    p.setEmail(rs.getString("email"));
                    // Gunakan nama kolom yang ada di DB, misal 'telepon' atau 'phone'
                    p.setPhone(rs.getString("telepon")); 
                    return p;
                }
            }
        } catch (SQLException e) {
        ...
        return null; // Jika ini yang jalan, maka JSP akan error 500
    }
}
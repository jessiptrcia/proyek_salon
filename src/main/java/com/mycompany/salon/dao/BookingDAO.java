...
public class BookingDAO {
    ...
    // Method untuk mengambil report berdasarkan filter
    public List<Booking> findReport(String filterType, String value) {
        List<Booking> list = new ArrayList<>();
        String sql = "";

        if (filterType.equals("harian")) {
            // Menggunakan DATE() untuk mengambil bagian tanggal saja dari timestamp
            sql = "SELECT * FROM booking WHERE DATE(tanggal_booking) = ? AND status = 'Completed'";
        } else if (filterType.equals("bulanan")) {
            sql = "SELECT * FROM booking WHERE DATE_FORMAT(tanggal_booking, '%Y-%m') = ? AND status = 'Completed'";
        } else if (filterType.equals("tahunan")) {
            sql = "SELECT * FROM booking WHERE YEAR(tanggal_booking) = ? AND status = 'Completed'";
        } else {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            if (!filterType.equals("all") && value != null && !value.isEmpty()) {
